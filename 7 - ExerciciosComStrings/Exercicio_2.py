...
def calcula_percentual_de_uso(bytes, total):
...
def main():
    ...
    with open('usuarios.txt', 'r') as arquivo:
        for linha in arquivo:
            usuario, bytes = linha.split()
            bytes = int(bytes)
            total += bytes
            usuarios.append((usuario, bytes))
    ...
    with open('relatorio.txt', 'w', encoding='utf-8') as arquivo:
        ...
        for i, (usuario, bytes) in enumerate(usuarios):
            arquivo.write(f'{i + 1:<4} {usuario:<15} {converte_bytes_para_megabytes(bytes):>10.2f} MB '
                          f'{calcula_percentual_de_uso(bytes, total):>10.2f}%\n')

        arquivo.write(f'\nEspaço total ocupado: {converte_bytes_para_megabytes(total):.2f} MB\n')
        arquivo.write(f'Espaço médio ocupado: {converte_bytes_para_megabytes(total / len(usuarios)):.2f} MB\n')
